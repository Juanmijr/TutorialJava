package capítulo4.ejercicioExtra02_La_Oca.version10;
...
public class Jugador {   
	...
	public void tirarDado () {
		...
		System.out.println("\tEst� en: " + this.casilla);

		// Si el jugador tiene una penalizaci�n activa no puede jugar en este
		// turno
		if (this.getTurnos() < 0) { // Hay penalizaci�n activa de turnos
			...
			// la sanci�n de turnos va poco a poco desapareciendo.
		}
		else { // El jugador puede jugar, no tiene penalizaci�n de turnos
			...
			// Una vez que tengo la nueva posici�n, tras tirar el dado, debo
			...
			// En el caso de que la nueva casilla tenga una situaci�n especial,
			// bonificaci�n o penalizaci�n, en el turno del jugador, se obtiene
			// dicha situaci�n y se almacena en el propio jugador.
			if (this.casilla.getTurnos() != 0) {
				this.turnos = this.casilla.getTurnos();
			}
			
			// Calculo la nueva posici�n por posibilidad de casilla
			// especial
			if (this.casilla.isEspecial()) { // En este caso hemos ca�do en una casilla especial
